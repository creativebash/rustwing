...
use serde_json::json;
use thiserror::Error;
use validator::ValidationErrors; // Import from your crate!

#[derive(Error, Debug)]
pub enum AppError {
    #[error(transparent)]
    Core(#[from] CoreError), // Wraps all core errors!

    #[error("Validation error: {0}")]
    Validation(#[from] ValidationErrors),
}

// Direct conversion from sqlx::Error to AppError
                    ...
                    if let Some(code) = db_err.code() {
                        match code.as_ref() {
                            ...
                            "23503" => {
                                return (
                                    StatusCode::CONFLICT,
                                    Json(json!({ "error": "Referenced resource not found" })),
                                )
                                    .into_response();
                            }
                            _ => {}
